<div class="row justify-content-center">
    <div class="col-12 col-sm-10 col-md-8">

        <div class="ft-10 mb-2">
            Selecione o ranking que você quer acessar:
        </div>

        <div class="row">

            <div class="col-12 col-sm-6 col-md-4 mb-2">
                <div class="card mb-1 shadow-md card-hover mpoint" data-eb-location="./ranking/saldo">
                    <div class="card-body py-1 ft-10">
                        <i class="bi bi-cash me-1"></i> SALDO
                        <br>
                        <img src="/images/ranking_icon_saldo.png" alt="Ranking Saldo" class="w-100 rounded my-1">
                    </div>
                </div>
            </div>

            <div class="col-12 col-sm-6 col-md-4 mb-2">
                <div class="card mb-1 shadow-md card-hover mpoint" data-eb-location="./ranking/transacoes">
                    <div class="card-body py-1 ft-10">
                        <i class="bi bi-graph-up-arrow me-1"></i> TRANSAÇÕES
                        <br>
                        <img src="/images/ranking_icon_transations.png" alt="Ranking Transações" class="w-100 rounded my-1">
                    </div>
                </div>
            </div>

            <div class="col-12 col-sm-6 col-md-4 mb-2">
                <div class="card mb-1 shadow-md card-hover mpoint" data-eb-location="./ranking/shop">
                    <div class="card-body py-1 ft-10">
                        <i class="bi bi-cart me-1"></i> SHOPPING
                        <br>
                        <img src="/images/ranking_icon_shop.png" alt="Ranking Shop" class="w-100 rounded my-1">
                    </div>
                </div>
            </div>

            <div class="col-12 col-sm-6 col-md-4 mb-2">
                <div class="card mb-1 shadow-md card-hover mpoint" data-eb-location="./ranking/cartao">
                    <div class="card-body py-1 ft-10">
                        <i class="bi bi-credit-card-2-front-fill me-1"></i> CARTÃO
                        <br>
                        <img src="/images/ranking_icon_card.png" alt="Ranking Cartão" class="w-100 rounded my-1">
                    </div>
                </div>
            </div>

            <div class="col-12 col-sm-6 col-md-4 mb-2">
                <div class="card mb-1 shadow-md card-hover mpoint" data-eb-location="./ranking/investimentos">
                    <div class="card-body py-1 ft-10">
                        <i class="bi bi-cash-coin me-1"></i> INVESTIMENTOS
                        <br>
                        <img src="/images/ranking_icon_investimentos.png" alt="Ranking Investimentos" class="w-100 rounded my-1">
                    </div>
                </div>
            </div>

            <div class="col-12 col-sm-6 col-md-4 mb-2">
                <div class="card mb-1 shadow-md card-hover mpoint" data-eb-location="./ranking/emprestimos">
                    <div class="card-body py-1 ft-10">
                        <i class="bi bi-suitcase-lg me-1"></i> EMPRÉSTIMOS
                        <br>
                        <img src="/images/ranking_icon_emprestimos.png" alt="Ranking Empréstimos" class="w-100 rounded my-1">
                    </div>
                </div>
            </div>

        </div>

    </div>
</div>